...
class OrderDetails::ParamUpdater
  ...
  def assign_price_changed_by_user
    ...
    elsif %w[actual_cost actual_subsidy adjust_cost price_change_reason].any? { |a| @order_detail.changed.include?(a) }
  ...
  def cost_params(params)
    params.slice(:actual_cost, :actual_subsidy, :adjust_cost).permit!
  end
