class window.UserSummaryPicker
  constructor: (@$form) ->
    @init_timeinput()
    @init_checkboxes()
...
class UserSummaryPickerDialog
  ...
  init_dialog_element: ->
    ...
    @dialog = $('#user_summary_dialog')

    # build dialog if necessary
    if @dialog.length == 0
      @dialog = $('<div id="user_summary_dialog" class="modal hide fade" data-backdrop="static" role="dialog"/>')
      @dialog.hide()
      $("body").append(@dialog);
...
$ ->
  $('body').on 'click', '.check_user', (evt) ->
    evt.preventDefault()
    picker = new UserSummaryPickerDialog($(this))

  new UserSummaryPicker($('.not-in-modal #user_summary-form'))
